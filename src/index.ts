...
import { shuffle, ankiMustache, chunk, generateSecret } from "./util";
...
export default class R2rMongo {
    ...
    public async insertMany(entries: IEntry[]): Promise<ObjectID[]> {
        ...
        await entries.filter((e) => e.sourceH).distinctBy((el) => el.sourceH!).mapAsync(async (el) => {
            try {
                sIdMap[el.sourceH!] = (await SourceModel.create({
                    name: el.source!,
    ...
    public async updateMany(ids: ObjectID[], u: Partial<IEntry>) {
        const cs = await (await CardModel.find({ _id: { $in: ids } })).mapAsync(async (c) => {
    ...
    public async deleteMany(ids: ObjectID[]) {
        return CardModel.deleteMany({_id: {$in: ids}});
    }
    ...
    public async markRight(cardId: ObjectID) {
        return await this.updateSrsLevel(+1, cardId);
    }

    public async markWrong(cardId: ObjectID) {
        return await this.updateSrsLevel(-1, cardId);
    }

    private async updateSrsLevel(dSrsLevel: number, cardId: ObjectID) {
    ...
    private async getOrCreateDeck(name: string): Promise<ObjectID> {
        try {
            return (await DeckModel.create({ name }))._id;
        } catch (e) {
            return (await DeckModel.findOne({ name }))!._id;
        }
    }

    private async getData(cardId: ObjectID): Promise<Record<string, any> | null> {
        const c = await CardModel.findOne({ _id: cardId }).populate("note", "data");
        if (c && c.note) {
            return (c.note as Note).data;
        }
    ...
    private async getFront(cardId: ObjectID): Promise<string> {
        const c = await CardModel.findOne({ _id: cardId }).populate("note", "data").populate("template", "front");
    ...
    public async fromAnki(
        anki: Anki, 
        options?: { filename?: string, callback?: (p: {
            text: string;
            current?: number;
            max?: number;
        }) => void }
